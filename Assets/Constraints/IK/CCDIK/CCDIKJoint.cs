...
public class CCDIKJoint : MonoBehaviour {
  ...
  public void Evaluate(Transform ToolTip, Transform Target, bool rotateToDirection = false) {
    ...
    transform.rotation = (rotateToDirection ? Quaternion.FromToRotation(ToolTip.forward, Target.forward) : Quaternion.FromToRotation(ToolTip.position - transform.position, Target.position - transform.position)) * transform.rotation;
